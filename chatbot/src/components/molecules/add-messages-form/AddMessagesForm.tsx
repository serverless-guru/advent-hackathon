...
interface AddMessagesFormProps {
  clearMessagesHandle: () => void;
  sendMessage: (message: string) => void;
}

const AddMessagesForm: React.FC<AddMessagesFormProps> = ({
  clearMessagesHandle,
  sendMessage,
}) => {
  const [value, setValue] = useState("");

  const sendMessagesHandle = () => {
    if (!value) {
      return;
    }
    sendMessage(value);
